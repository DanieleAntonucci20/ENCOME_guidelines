...
Overview:
  ...
  - Docker container with PostgreSQL dabase (not necessarily running on the same VM)
  - Encome app: multiple Docker containers (fron-end, back-end, celery, redis) 
  - Geoclustering Tool: multiple Docker containers (front-end, back-end, eebdatabase)
  - Docker container with NGINX
  - all containers(except database if running on external server) use the same Docker network.


## Prerequisites
In order to performe deployment procedure, you need a GitLab account and a Docker installed on your local machine. 
...
From GitLab, create new Project Access Token and note it somwhere (https://docs.gitlab.com/ee/user/project/settings/project_access_tokens.html).
...
```
    ...
    ssh <your-username>@193.106.181.3
```
...
```
    rsync -r ./nginx <your-username>@193.106.181.3:/home/<your-username>/ 
```
...
First, from local and server terminal login to GitLab (\@ref(gitlab-login)).

From local terminal cd into ./iso52016_database/database and run:
``` 
    # build  
...
Then, from local terminal cd into ./iso52016_database/migration_app and run:
```
    # build
...
```
    rsync -r ./scripts <your-username>@xdcren06.eurac.edu:/home/<your-username>/iso52016_database/
    rsync docker-compose.yml <your-username>@xdcren06.eurac.edu:/home/<your-username>/iso52016_database/
    rsync db.env <your-username>@xdcren06.eurac.edu:/home/<your-username>/iso52016_database/
```
...
So, run:
```
    rsync docker-compose.yml <your-username>@193.106.181.3:/home/<your-username>/ 
    rsync .env <your-username>@193.106.181.3:/home/<your-username>/ 
```
...
From local terminal:
  ...
  - login to GitLab
  - build fronted or/and backend image/s
